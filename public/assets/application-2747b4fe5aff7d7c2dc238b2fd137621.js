...
(function() {
  $(function() {
    ...
    $(".tags_field").bind("keydown", function(event) {
...
(function() {
  ...
  startBookmarkAt = function(start_at) {
    var audio, event, startTime;
    ...
    if (audio.readyState >= 3) {
      ...
      event = function() {
        return startBookmarkAt(start_at);
      };
      return window.queuedEvents.push(event);
    }
  };
  ...
  $(document).ready(function() {
    var addBookmark, dialog, form;
    if ($("#bookmark").length > 0) {
      startBookmarkAt($("#bookmark"));
    }
    $(".add-bookmark").click(function() {
      var audio, media_file;
      dialog.dialog("open");
      media_file = $(".add-bookmark").data("audio-file");
      audio = $("audio")[0];
      return audio.pause();
    });
    $(".audio-bookmark").click(function() {
      return startBookmarkAt($(this));
    });
    $("audio").bind("canplay", function() {
      return _(queuedEvents).forEach(function(event) {
        return event();
      });
    });
    $(".start_at").each(function(item) {
      return startBookmarkAt($(this));
    });
    addBookmark = function() {
      var audio, data, form, media_file;
      audio = $("audio")[0];
      media_file = $(".add-bookmark").data("audio-file");
      form = dialog.find("form");
      $("input[name=startTime]").val(audio.currentTime);
      data = form.serializeObject();
      return $.ajax("/add_bookmark", {
        type: "POST",
        data: data,
    ...
    return form = dialog.find("form").on("submit", function(event) {
      event.preventDefault();
    });
  });
